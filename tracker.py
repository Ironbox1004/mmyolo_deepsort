...
def update(pred_instances, image, dataset_classes):

    bboxes2draw = []

    if len(pred_instances) > 0:
        bboxes = pred_instances.bboxes.data
        confss = pred_instances.scores.data

        x1, y1, x2, y2 = bboxes.split((1, 1, 1, 1), dim=-1)
        bbox_new = [(x1 + x2) / 2, (y1 + y2) / 2, (x2 - x1), (y2 - y1)]
        bbox_xywh = torch.cat(bbox_new, dim=-1)
        ...
        for x1, y1, x2, y2, track_id in list(outputs):
            ...
            label = search_label(center_x=center_x, center_y=center_y,
                                 pred_instances=pred_instances, max_dist_threshold=20.0, dataset_classes=dataset_classes)
...
def search_label(center_x, center_y, pred_instances, max_dist_threshold, dataset_classes):
    """
    ...
    bboxes = pred_instances.bboxes.data
    labels = pred_instances.labels.data

    for i, bbox in enumerate(bboxes):
        center_x2 = (bbox[0] + bbox[2]) * 0.5
        center_y2 = (bbox[1] + bbox[3]) * 0.5
        ...
        if min_x < max_dist_threshold and min_y < max_dist_threshold:
            ...
            if min_dist == -1.0:
                ...
                min_dist = avg_dist
                # 赋值label
                label = str(dataset_classes[int(labels[i])])
                pass
            else:
                # 若不是第一次，则距离小的优先
